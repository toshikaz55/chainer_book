...
while len(next_page) > 0:
    ...
    with urllib.request.urlopen(url) as response:
        ...
        # ギャラリー表示部分のタグを取得する
        gallery = re.findall( \
            r'<div class=\"gallerytext\">\s+<a\s+href=\"/wiki/File:(\S*)\"', html, re.DOTALL)
        ...
        for g in gallery:
            ...
            with urllib.request.urlopen(suburl + g) as response:
                ...
                original = re.findall(\
                    r'<a\s+(?:class=\"internal\")?\s*href=\"(https://upload.wikimedia.org/\S*/'+g+')\"[\s\S]*>[\s\S]*</a>', html)
                ...
                for o in original:
                    ...
                    if face.endswith('.tif') or face.endswith('.tiff'):
                        os.system('gdal_translate -of JPEG -ot Byte -co QUALITY=100' + \
                            ' portrait/'+face+' portrait/'+face+'.jpg')
